...
% db.runCommand({count: 'r_gripper_controller__command', query: {'position': 0.0}})
% { "n" : 3, "ok" : 1 }
% millis: 0

% Get the amount of time where the camera was behind the robot.
% db.runCommand({count: 'rviz_camera_publisher__camera_pose', query: {'position.x': {$lte: 0}}})
% { "n" : 1292, "ok" : 1 }
% millis: 19

% Get the number of times the right gripper was moved in a particular region
% db.runCommand({count: 'r_cart__command_pose', query: {'pose.position.z': {$gt: 1}, 'pose.position.y': {$gt: -0.5, $lt: 0.5}, 'pose.position.x': {$gt: 0.5, $lt: 1}}})
% millis: 2
...
% SQLite3:
%
%   --Count msgs where position.x <= 0
%   SELECT COUNT(*) 
%   FROM rviz_camera_publisher__camera_pose
%   WHERE json_le_field(msg, 'position.x', 0);
%
%   result: 1292, 124.76ms (3179.27ms on first run)
%
%   --Count msgs where position.x >= 0
%   SELECT COUNT(*) 
%   FROM rviz_camera_publisher__camera_pose
%   WHERE json_ge_field(msg, 'position.x', 0);
%
%   result: 11275, 121.25ms
%
%   --Count msgs where header.stamp.secs >= 1400729704
%   SELECT COUNT(*) 
%   FROM head_mount_kinect__rgb__image_color__compressed
%   WHERE json_ge_field(msg, header.stamp.secs, 1400729704);
%
%   result: 8678, 2074.9ms
